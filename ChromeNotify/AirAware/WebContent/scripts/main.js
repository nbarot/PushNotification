...
var API_KEY = 'AIzaSyBwgDylmvNVSB6dNRE81hswyyTpdbPq8ZU';//'AIzaSyC8g2PYqtC1bUZiYBFY5ekR19OX7ylI7fA';
var PUSH_SERVER_URL = './';
...
function onPushSubscription(pushSubscription) {
  ...
  var frqFlyerNumber = document.querySelector('.js-frequent-flyer-number');
  ...
  sendPushViaXHRButton.addEventListener('click', function(e) {
    ...
    if ('subscriptionId' in pushSubscription) {
      ...
      if (!endpoint.includes(pushSubscription.subscriptionId)) {
        ...
        endpoint += '|' + frqFlyerNumber.value;
        endpoint += '|';
      }
    }
    
    navigator.serviceWorker.controller.postMessage({
        command: frqFlyerNumber.value
    });
    
    //getIdb().put(KEY_VALUE_STORE_NAME, "FFP", frqFlyerNumber.value);
    ...
    fetch(PUSH_SERVER_URL + 'subscribe', {
        ...
        window.location.href = "subscribeMessage.html";        
      }).catch(function(err) {
